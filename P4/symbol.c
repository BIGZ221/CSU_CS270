...
sym_table_t* symbol_init (int capacity) {
    sym_table_t* newTable = malloc(sizeof(sym_table_t));
    newTable->capacity = capacity;
    newTable->hash_table = calloc(sizeof(node_t*), capacity);
    newTable->addr_table = calloc(sizeof(char*), LC3_MEMORY_SIZE);
  return newTable;
}
...
int symbol_add (sym_table_t* symTab, const char* name, int addr) {
    int hash;
    int index;
    node_t* head = symbol_search(symTab, name, &hash, &index);
    if (head == NULL) {
        ...
        symTab->addr_table[addr] = newName;
        return 1;
    }
    return 0;
}
...
struct node* symbol_search (sym_table_t* symTab, const char* name, int* hash, int* index) {
  ...
  *index = *hash % symTab->capacity;
  ...
  while (head != NULL) {
      if (head->hash == *hash) {
          if (strcasecmp(head->symbol.name, name) == 0) {
              return head;
        }
      }
      head = head->next;
